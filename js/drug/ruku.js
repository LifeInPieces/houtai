$(function(){
  ...
  // 渲染数据
  function getdrugRUkusData() {
    $.ajax({
      url: http + "/buydrugs/findAll.do",
      type: "get",
      dataType: "json",
      data: {
        page: page,
  ...
  $(".draw_add").click(function(){
    ...
    formData.append("buydrugId",buydrugId);
    $.ajax({
      url: http + "/buydrugs/uploadimg.do",
      type: "post",
      dataType: "json",
      cache: false,
      processData: false,
  ...
  function getData() {
   ...
   $(".preserve_add").click(function () {
    for (var i = 0; i < document.newUser.elements.length - 1; i++) {
      if ($("#buynumber").val() == "" || $("#date").val() == "" || $("#purchaser").val() == "" ||$("#buycompany").val() == ""
       || $("#payable").val() == "" || $("#payamount").val() == "" || $("#amountowed").val() == "" ) {
        toastr.options.positionClass = 'toast-center-center';
        toastr.options.timeOut = '3000';
...
$(".buttonDate").click(function () {
    ...
    $.ajax({
        type: "get",
        url: http + "/buydrugs/findByDate.do",
        dataType: "json",
        data: {
 ...
 $(".btn_found").click(function () {
    var purchaser = $("#purchaser_index").val()
    // 数据为空时不能查询
    if (purchaser == "") {
      return false;
    }
    ...
    // 点击进入入库明细
    $("#rukuTable").delegate(".drugDetails", "click", function () {
        var buydrugId = $(this).data("id")
        localStorage.setItem("buydrugId", buydrugId);
